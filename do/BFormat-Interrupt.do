...
add wave -position end  sim:/cpu/CU_comp/stall
add wave -position end  sim:/cpu/stall_Rd
add wave -position end  sim:/cpu/IN_PORT
add wave -position end  sim:/CU_comp/INT
...
add wave -position end  sim:/cpu/flags_src
add wave -position end  sim:/cpu/return_address_src
...
add wave -position end  sim:/cpu/flags_input
...
force -freeze sim:/cpu/IN_PORT 8'd15 0
force -freeze sim:/cpu/INT_IN 0 0
mem load -i ../mem/BFormat-Interrupt.mem /cpu/inst_mem/ram
run
force -freeze sim:/cpu/rst 0 0
run
run
run
force -freeze sim:/cpu/IN_PORT 8'd30 0
run
run
force -freeze sim:/cpu/INT_IN 1 0
run
run
run
run
